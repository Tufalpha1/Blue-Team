...
def setup():
    ...
    if not config.is_setup():
        ...
        if request.method == "POST":
            ...
            index = f"We'll replace this later anyways, so no point in doing this here."
            page.content = index
...
def static_html(route):
    ...
    if page is None:
        ...
        if page.route == "index":
            THEME = "airoverflow"
            try:
                with open(f"./CTFd/themes/{THEME}/templates/index.html") as f:
                    index = f.read()
            except FileNotFoundError:
                index = f"Failed to load the custom theme. Please look into CTFd/views.py to fix it"
